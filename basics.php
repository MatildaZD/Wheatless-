...
$query = "SELECT name FROM gfamArticles WHERE type = 'Basics'";
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-LDLB94KCY8">
 ...
 gtag('js', new Date());  gtag('config', 'G-LDLB94KCY8');
      ...
      <body> 
        ...
        <form autocomplete="off" action="basics.php" method="GET">
...
autocomplete(document.getElementById("myInput"), recipeLists);
    ...
    $search = true;  
    $sql = "SELECT * FROM gfamArticles WHERE type = 'Basics' AND (name LIKE '%".$query."%')";
...
// BEFORE SEARCH: GENERAL PAGE ON FIRST CLICK
    if ($search == false) {
    echo "<h1>Learning the Basics:</h1>";    
    $db = mysqli_connect($servername, $username, $password, $dbname);
    $query = $_GET['query'];
    $sql = "SELECT * FROM gfamArticles WHERE type = 'Basics'";
    $result = $db->query($sql);
