...
$query = "SELECT recipe FROM recipes WHERE meal = 'breakfast'";
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-LDLB94KCY8">
 ...
 gtag('js', new Date());  gtag('config', 'G-LDLB94KCY8');
      ...
      <body> 
        <span id='enter'>Search for a Recipe:</span>
        <form autocomplete="off" action="breakfast.php" method="GET">
...
autocomplete(document.getElementById("myInput"), recipeLists);
    ...
    $search = true;  
    $sql = "SELECT * FROM recipes WHERE meal = 'breakfast' AND (recipe LIKE '%".$query."%')";
    $result = $db->query($sql);
    ...
    for ($j = 0; $j < 2; $j++) {
        if ($i + $j < $num_rows) {
            ...
            echo "<td style='border-radius: 20px; padding: 20px; background-color: #f3e1a1; text-align: center;'>" .
                 "<div style='margin-bottom: 10px;'><strong>" . $row["recipe"] . "</strong></div>" .
          ...
          "<span class='close'>&times;</span>".
   				...
   				"<p>
				<strong style='font-size: 30px;'>" . $row["recipe"] . "</strong><br><br>
				 
          <a href='" . $row["url"] . "' target='_blank'>Link to Recipe</a><br>
...
// BEFORE SEARCH: GENERAL PAGE ON FIRST CLICK
    if ($search == false) {
    echo "<h1>Our Breakfast Recipes:</h1>";    
    $db = mysqli_connect($servername, $username, $password, $dbname);
    $query = $_GET['query'];
    $sql = "SELECT * FROM recipes WHERE meal = 'breakfast'";
    $result = $db->query($sql);
    ...
    for ($j = 0; $j < 2; $j++) {
        if ($i + $j < $num_rows) {
            ...
            echo "<td style='border-radius: 20px; padding: 20px; background-color: #f3e1a1; text-align: center;'>" .
                 ...
                 //  "<button class = 'buttons' id='myBtn" . $row["id"] . "'>Info</button>".
                 // "<div id='myModal"  . $row["id"] . "'class='modal'>"  . 


                  "<button class = 'buttons' id='myBtn" . $row["id"] . "'><img src = 'infobut.png' style = 'width:30px; height: 30px;'></button>".
                "<div id='myModal"  . $row["id"] . "'class='modal'>"  . 
                "<div class='modal-content'>".
          "<span class='close'>&times;</span>".
   				"<p><strong style='font-size: 30px;'>" . $row["recipe"] . "</strong><br><br>
