...
$query = "SELECT recipe FROM recipes WHERE meal = 'desserts'";
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-LDLB94KCY8">
 ...
 gtag('js', new Date());  gtag('config', 'G-LDLB94KCY8');
      ...
      <body> 
        ...
        <form autocomplete="off" action="dessert.php" method="GET">
...
autocomplete(document.getElementById("myInput"), recipeLists);
    ...
    $search = true;  
    $sql = "SELECT * FROM recipes WHERE meal = 'desserts' AND (recipe LIKE '%".$query."%')";
    $result = $db->query($sql);
...
// BEFORE SEARCH: GENERAL PAGE ON FIRST CLICK
    ...
    $db = mysqli_connect($servername, $username, $password, $dbname);
    $query = $_GET['query'];
    $sql = "SELECT * FROM recipes WHERE meal = 'desserts'";
    $result = $db->query($sql);
