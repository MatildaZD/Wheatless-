...
$query = "SELECT recipe FROM recipes WHERE meal = 'lunch-dinner'";
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-LDLB94KCY8">
 ...
 gtag('js', new Date());  gtag('config', 'G-LDLB94KCY8');
      ...
      <body> 
        ...
        <form autocomplete="off" action="dinner.php" method="GET">
...
autocomplete(document.getElementById("myInput"), recipeLists);
    ...
    $search = true;  
    $sql = "SELECT * FROM recipes WHERE meal = 'lunch-dinner' AND (recipe LIKE '%".$query."%')";
    $result = $db->query($sql);
...
// BEFORE SEARCH: GENERAL PAGE ON FIRST CLICK
    if ($search == false) {
    echo "<h1>Our Lunch and Dinner Recipes:</h1>";    
    $db = mysqli_connect($servername, $username, $password, $dbname);
    $query = $_GET['query'];
    $sql = "SELECT * FROM recipes WHERE meal = 'lunch-dinner'";
    $result = $db->query($sql);
